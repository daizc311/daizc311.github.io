...
comments: true
cover: https://oss.note.dreamccc.cn/note/images/posts/集群爆炸，但是修好了/title.png?x-oss-process=style/blog_title
categories:
- [我永远爱学习]
tags:
- Kubernetes
...
1. 先看docker ps,容器全都停在pause上,kubectl get node没有反应
   ```shell
   ...
   ```
2. 求助运维老哥，老哥叫看了下现场，说可能是etcd爆炸了，叫检查下etcd。因为使用kubesphere脚本部署的，所以也不知道etcd怎么启动的，检查了一圈，没有发现etcd的影子。
